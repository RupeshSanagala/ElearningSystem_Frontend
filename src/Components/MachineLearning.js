...
const MachineLearning = () => {
  ...
  const images = [
     constantData.mlIntro,
     constantData.neuralNetworks,
     constantData.svm, 
     constantData.regression, 
     constantData.kmeans,
     constantData.deployment
  ];
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await HttpService.get("/Course"); // Fetch all courses
        const MachineLearningCourses = response.data.filter(
          (course) => course.category === "Machine Learning"
        );
        setCourses(MachineLearningCourses); // Set only Web Development courses
      } catch (err) {
